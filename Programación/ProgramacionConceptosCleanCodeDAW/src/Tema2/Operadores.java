...
public class Operadores {
    ...
    // 1. OPERADORES ARITMÉTICOS (metodo: operadoresAritmeticos)
    /* Unarios: actúan sobre un solo operando
       ...
       * multiplicación
       ...
       Uso de printf para mostrar decimales formateados: %.2f   */

    // 2. OPERADORES DE ASIGNACIÓN (operadoresAsignacion)
    /* x += y -> x = x + y
       x -= y -> x = x - y
       x *= y -> x = x * y
       x /= y -> x = x / y
       ...
       Muy usados para acortar operaciones acumulativas. */

    // 3. OPERADORES DE COMPARACIÓN (operadoresComparacion)
    /* Devuelven true o false:
       ...
       Son fundamentales para condiciones y estructuras lógicas.  */

    // 4. OPERADORES LÓGICOS (operadoresLogicos)
    /* Se usan para combinar varias condiciones booleanas.
       && AND -> todas deben ser true
       || OR -> basta con que una sea true
       ! NOT -> invierte el valor booleano

       Muy usados en validaciones y estructuras de control. */

    // 5. USO DE SCANNER PARA LEER DATOS
    /* Scanner lector = new Scanner(System.in);
       ...
       Siempre cerrar el scanner cuando ya no se use: lector.close();   */

    // 6. EJEMPLO PRÁCTICO: evaluar un candidato (evaluarCandidato)
    /* Se pide:
       Nombre
       Edad
       Salario deseado
       ...
       Condición evaluada:
       edad < 40 && salario < 40000 && experiencia
       Solo será true si las tres condiciones se cumplen. */

    // 7. CONVERSIONES DE TIPOS (cambiosTipo)
    /* Conversión con valueOf y parse:
       String.valueOf(variable) -> cualquier tipo a String
       Boolean.parseBoolean("true") -> String a boolean
       Casting explícito:
       (double) valor
       Ejemplo en clase:
       char letra = 'a';
       String.valueOf(letra) + 123 */

    // 8. CLASE Entrada: cómo ejecutar los métodos del tema
    /* En main se crea un objeto y se llaman los métodos:

       op.operadoresAritmeticos();
       op.operadoresComparacion();
       op.operadoresLogicos();
       op.evaluarCandidato();
       op.cambiosTipo();

       Esto ejecuta todas las explicaciones del tema de operadores. */
